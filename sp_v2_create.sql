------------------------------------------------------------------
-- Versión: V2_CV Corregida (Auditoría Integral)
-- Fecha: 11/03/2026
------------------------------------------------------------------
CREATE PROCEDURE [dbo].[RepFormatoFacturaVentaOM_Consolidada_V2_CV]
...
AS 
    BEGIN
        ...
        SELECT
            ...
            -- FIX C: Partición por factura para no cruzar numeración entre documentos
            , ROW_NUMBER() OVER(PARTITION BY FV.doc_num ORDER BY FVR.co_art ASC) AS reng_num
            
            , FVR.co_art
            , ART.art_des
            
            -- FIX A: Evitar división por cero con NULLIF
            , SUM(CASE WHEN FVR.co_uni = 'UNI' THEN FVR.total_art ELSE 
                CASE WHEN UNI.relacion = 0 THEN (FVR.total_art * UNI.equivalencia) ELSE (FVR.total_art / NULLIF(UNI.equivalencia, 0)) END 
              END) AS total_art
              
            , UNI.equivalencia
            
            -- FIX D: Evitar multiplicación de peso por la agrupación (SUM a MAX)
            , MAX(ART.stock_pedido) AS peso
            ...
            -- FIX B: Considerar descuento usando reng_neto en vez de Precio*Cant
            ---------------------------------------------------------
            ...
            , SUM(CASE WHEN FVR.tipo_imp = 5 
                       THEN FVR.reng_neto / NULLIF(FV.tasa, 0) 
        ...
        GROUP BY
            ...
            , FV.tasa, FV.porc_reca, FV.monto_reca, FV.otros3, FV.dir_ent, FV.comentario, FVR.doc_num, FVR.co_art, ART.art_des, UNI.equivalencia
