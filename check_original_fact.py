...
def run():
    ...
    print("=" * 80)
    print("DETALLES DE LA FACTURA ORIGINAL saFacturaVenta 0000010161")
    print("=" * 80)
    sql_fact_header = """
        SELECT doc_num, total_bruto, total_neto, monto_imp, saldo, anulado
        FROM saFacturaVenta
        WHERE doc_num = '0000010161'
    """
    print(sqlcmd(client, sql_fact_header))

    print("\n" + "=" * 80)
    # Renglones de saFacturaVentaReng for 0000010161
    sql_fact_rengs = """
        SELECT reng_num, co_art, total_art, prec_vta, porc_desc, monto_desc, reng_neto, porc_imp
        FROM saFacturaVentaReng
        WHERE doc_num = '0000010161'
        ORDER BY reng_num
    """
    print(sqlcmd(client, sql_fact_rengs))
